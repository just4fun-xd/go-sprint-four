...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	calories := (walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeed(action, duration)*kmhInMsec, 2)/
		(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH
	return calories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	calories := (swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) *
